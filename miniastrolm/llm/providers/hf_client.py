...
from miniastrolm.llm.base import LLMClient, LLMResult, LLMUsage
...
class HFLLMClient(LLMClient):
    ...
    def __init__(self, model_name: str | None = None,
        ...
        model_name = model_name or os.getenv("HF_MODEL_NAME", "meta-llama/Meta-Llama-3.1-8B-Instruct")
        token = token or os.getenv("HF_TOKEN") or os.getenv("HUGGING_FACE_HUB_TOKEN")
        device_map = os.getenv("HF_DEVICE_MAP", device_map)
        if torch_dtype is None:
            torch_dtype = torch.float16 if torch.cuda.is_available() else torch.float32
        if not torch.cuda.is_available() and device_map == "auto":
            # Avoid accelerate deciding to offload layers to disk on CPU-only runs.
            device_map = "cpu"
