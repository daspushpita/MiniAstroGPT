...
# -------------------------
# Prompt builders (chat-native)
# -------------------------
class JudgePromptBuilder:
    """
    Loads a rules-only system prompt from prompt_path, then injects instance data
    (paper_id/abstract/explanation) into the user message.
    """
    def __init__(self, prompt_path: Path):
        self.my_config = JudgeConfig()
        self.base_system_template = prompt_path.read_text(encoding="utf-8").strip()
        self.end_sentinel = "<END_JSON>"

    def build(self, *, paper_id: str, abstract: str, explanation: str) -> ChatPrompt:
        # Keep system as rules (ideally no {abstract}/{explanation} in the template).
        system_prompt = self.base_system_template.format(paper_id=paper_id, abstract=abstract, explanation=explanation)
        ...
        user_prompt = (
            f"paper_id: {paper_id}\n\n"
            f"SOURCE OF TRUTH (ABSTRACT):\n{abstract}\n\n"
            f"TEACHER EXPLANATION TO EVALUATE:\n{explanation}\n\n"
            "Return ONLY one JSON object with EXACT keys: "
            "id, scores, error_tags, rationale, rewrite_hint. "
            "id must equal paper_id."
        )

        # Optional sentinel (only if you actually use it in extraction; see extractor below).
        if self.my_config.arch == "llama_cpp":
            user_prompt += (
                "\n"
        ...
        return ChatPrompt(system=system_prompt, user=user_prompt)

class JudgeRepairPromptBuilder_v1:
...
class JudgeValidator:
    ...
    def _check_required_keys(self, obj: Dict[str, Any]) -> tuple[bool, str]:
        req_keys = self.my_config.REQUIRED_TOP_KEYS
    ...
    def _check_scores(self, scores: Dict[str, Any]) -> tuple[bool, str]:
        ...
        score_keys = self.my_config.REQUIRED_SCORE_KEYS
    ...
    def _check_error_tags(self, error_tags: Any) -> tuple[bool, str]:
        ...
        bad = [t for t in error_tags if t.split(":", 1)[0].strip().lower() not in self.my_config.CANONICAL_TAGS]
        if bad:
...
class LLMJudge:
    ...
    def __init__(self, llm_client: Any,
                ...
                llm_config: TeacherConfig | None = None,
                ...
                re_eval_prompt_builder: JudgeReevalPromptBuilder | None = None,
                max_attempts: int = 3):
        
        self.llm_client = llm_client
        self.judge_config = judge_config or JudgeConfig()
        self.llm_config = TeacherConfig()
        self.prompt_builder = prompt_builder or JudgePromptBuilder(prompt_path)
        self.validator = validator or JudgeValidator()
        self.repair_prompt_builder = repair_prompt_builder or JudgeRepairPromptBuilder_v2()
        self.re_eval_prompt_builder = re_eval_prompt_builder or JudgeReevalPromptBuilder()
    ...
    def judge_one(self, *, paper_id: str, abstract: str, explanation: str) -> Dict[str, Any]:
        ...
        chat = self.prompt_builder.build(
            paper_id=paper_id,
            abstract=abstract,
        ...
        while attempt < self.max_attempts:
            raw_output = self.llm_client.generate_response_chat(
                system_prompt=chat.system,
                user_prompt=chat.user,
                max_new_tokens=self.judge_config.max_new_tokens,
                ...
                do_sample=self.judge_config.do_sample,
            )
            ...
            if needs_reeval:
                ...
                reeval_user = self.re_eval_prompt_builder.build(
                                    paper_id=paper_id,
                                    abstract=abstract,
                                    explanation=explanation,
                                    reason=reason,
                                )
                chat = ChatPrompt(system=chat.system, user=reeval_user)
            else:
                repair_user = self.repair_prompt_builder.build(
                                paper_id=paper_id,
                                errors=last_errors,
                                bad_output=raw_output,
                                )
                chat = ChatPrompt(system=chat.system, user=repair_user)
        return {
            "id": paper_id,
            "judge": None,
            "accepted": False,
    ...
    def _accept_policy(self, judge_obj: Dict[str, Any]) -> bool:
        ...
        return (faith >= self.judge_config.min_faithfulness_to_keep) and (overall >= self.judge_config.min_overall_to_keep)
...
class JudgeJsonExtractor:
    ...
    def iter_json_object_strings(self, text: str) -> Iterator[str]:
        """
    ...
    def extract_obj(self) -> Optional[Dict[str, Any]]:
        ...
        if "<END_JSON>" in text:
            text = text.split("<END_JSON>", 1)[0].strip()
        if text.startswith("{") and text.endswith("}"):
            try:
                ...
                pass
        for json_str in self.iter_json_object_strings(text):
            try:
