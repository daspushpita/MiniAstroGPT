...
class AstroAgent:
    ...
    def _critic_passes(self, critic_payload: dict[str, Any]) -> bool:
        ...
        hall = int(scores.get("hallucination", 5))
        struct = int(scores.get("structure", 5))
        clar = int(scores.get("clarity", 5))
        return (hall <= 1) and (struct <= 2) and (clar <= 2)
    ...
    def run(self, abstract: str) -> AgentRun:
        ...
        for _ in range(self.max_revision_attempts):
