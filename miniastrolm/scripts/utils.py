...
class AgentConfig:
    ...
    fast_max_new_tokens: int = 1024
    write_mode: bool = True

# === A) Daily data ===
def get_daily_paths_and_date(my_config: AgentConfig):
    ...
    cache_path = CACHE_DIR / "papers.jsonl"
    return raw_path, merged_path, cleaned_path, db_output_path, json_output_path, cache_path, formatted_date_start, formatted_date_end
...
def cleanup_session_artifacts(
    ...
    db_output_path: Optional[Path] = None,
    json_output_path: Optional[Path] = None,
    remove_empty_dirs: bool = True,
    extra_dirs: Optional[Iterable[Path]] = None,
) -> list[str]:
    removed_files: list[str] = []
    dirs_to_maybe_remove: set[Path] = set()
    for file_path in [raw_path, merged_path, cleaned_path, db_output_path, json_output_path]:
        if file_path is None:
            continue
        path_obj = Path(file_path)
        dirs_to_maybe_remove.add(path_obj.parent)
        try:
    ...
    if extra_dirs:
        for dir_path in extra_dirs:
            if dir_path is not None:
                dirs_to_maybe_remove.add(Path(dir_path))

    if remove_empty_dirs:
        for dir_path in sorted(dirs_to_maybe_remove, key=lambda p: len(p.parts), reverse=True):
            try:
...
def run_agent_once(config: AgentConfig, agent: AstroAgent, 
                abstract: str, paper_id: str, title: str,
                debug: bool, cache_path: Path,
                batch_mode: bool,
                ):
    """
    ...
    if config.write_mode and not batch_mode:
        record = {
            "id": paper_id,
            "title": title,
            "abstract": abstract,
            "mode": getattr(run_result, "mode", None),
            ...
            "final_explanation": getattr(run_result, "revised_draft", None),
...
def run_agent_batch(config: AgentConfig, 
    ...
    if config.write_mode:
        with open(cache_path, "a") as f_out:
            for paper_id, title, abstract in batch_items:
                try:
                    run_result = run_agent_once(config=config, agent=agent, 
                                                abstract=abstract, paper_id=paper_id, title=title,
                                                debug=debug, cache_path=cache_path, batch_mode=True)
...
def orchestration(config: AgentConfig, 
    ...
    if mode == "single":
        ...
        return run_agent_once(config=config, agent=my_agent, 
                            abstract=abstract, paper_id=paper_id,
                            title=title,
                            debug=debug, cache_path=cache_path,
                            batch_mode=False)
        ...
        return run_agent_batch(config=config, agent=my_agent, 
                            batch_items=batch_items, debug=debug,
